...
```bash
sed -i 's|https://127.0.0.1:6443|https://cluster-api.internal.example:6443|g' ~/.kube/config
```
...
- Headlamp dashboard: `https://headlamp.internal.example`
- Spark History Server: `https://spark-history.internal.example`
- Prometheus: `https://prometheus.internal.example`
- Grafana: `https://grafana.internal.example`
...
1. Verify the cluster API endpoint is correct: `https://cluster-api.internal.example:6443`
2. Check DNS resolution: `host cluster-api.internal.example`
...
2. Verify the `cluster-api.internal.example` hostname matches the TLS SAN in the k3s configuration
