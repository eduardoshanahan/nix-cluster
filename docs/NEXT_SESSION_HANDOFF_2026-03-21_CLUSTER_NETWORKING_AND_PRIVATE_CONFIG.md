...
- Prometheus on `pi-node-b` scrapes `kube-state-metrics` through
...
`pi-node-b` monitoring now targets:
...
Verified from generated desired monitor JSON on `pi-node-b`:
...
```bash
ssh -o BatchMode=yes -o ConnectTimeout=6 pi-node-b \
