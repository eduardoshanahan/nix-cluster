...
- `cluster-node-01` was deployed with firewall port `30081` open.
...
- `curl -I http://headlamp.<homelab-domain>:30081/` also returned `HTTP/1.1 200 OK` from `cluster-node-01`.
- `localhost:30081` on `cluster-node-01` did not answer, but the node IP and FQDN did. That did not block the working path.

## Open Issue To Investigate

- From another host, `http://headlamp.<homelab-domain>:30081/` timed out.
- Since the service works from this machine and from `cluster-node-01`, the likely causes are:
