...
- Traefik now sits behind MetalLB on `192.168.1.36`.
- Pi-hole should point ingress hostnames at `192.168.1.36`, not at a node IP.
...
curl -I --max-time 10 http://192.168.1.36/
curl -k -I --max-time 10 https://192.168.1.36/ -H 'Host: headlamp.<homelab-domain>'
```
...
```bash
ssh -F /dev/null -o StrictHostKeyChecking=accept-new -o BatchMode=yes eduardo@192.168.1.10 \
...
ssh -F /dev/null -o StrictHostKeyChecking=accept-new -o BatchMode=yes eduardo@192.168.1.31 \
...
ssh -F /dev/null -o StrictHostKeyChecking=accept-new -o BatchMode=yes eduardo@192.168.1.31 \
  'sudo k3s kubectl -n headlamp create secret tls homelab-wildcard-tls \
...
```bash
ssh -F /dev/null -o StrictHostKeyChecking=accept-new -o BatchMode=yes -o ConnectTimeout=5 eduardo@192.168.1.31 \
...
- If the hostname fails but `192.168.1.36` works, it is a DNS issue.
- If both hostname and `192.168.1.36` fail, it is a network or ingress issue.
