...
and using `pi-node-a` (`192.0.2.58`) as the shared ARM builder.
...
- `pi-node-a` is not signing its outputs today
...
- `pi-node-a` signs its locally built outputs
- cluster nodes trust the `pi-node-a` public signing key
...
- `pi-node-a` remains at `192.0.2.58`
...
Before relying on the cross-host path, verify these facts on `pi-node-a`:
...
1. deploy the intended config with `pi-node-a` as build host
