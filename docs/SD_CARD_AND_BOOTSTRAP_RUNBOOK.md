...
4. Boot `cluster-node-01` first.
5. Verify SSH access and `k3s` server health on `cluster-node-01`.
6. Boot `cluster-node-02` and `cluster-node-03`.
7. Verify control-plane quorum and node readiness.
8. Boot `cluster-node-04` and `cluster-node-05`.
