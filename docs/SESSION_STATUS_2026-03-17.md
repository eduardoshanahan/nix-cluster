...
NIX_SSHOPTS='-F /dev/null -o StrictHostKeyChecking=accept-new -o BatchMode=yes -o ConnectTimeout=5' \
  ...
  --flake 'path:/home/eduardo/Programming/gitea.<homelab-domain>/nix-cluster#cluster-node-01' \
...
### 3. `pi-node-a` is not currently usable as a fully trusted remote builder

We verified on `pi-node-a`:
...
- `pi-node-a` is **not** currently signing locally built outputs
- targets will reject store paths copied from `pi-node-a`
