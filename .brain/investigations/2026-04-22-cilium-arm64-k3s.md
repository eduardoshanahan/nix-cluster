...
All nodes on 192.0.2.0/24 L2 subnet.
...
k8sServiceHost: cluster-api.<homelab-domain>
k8sServicePort: 6443
```
...
- Before SNAT: source = `192.0.2.34` (host IP), dest = `10.42.5.66` (pod IP)
...
de-NAT it back to `192.0.2.34` — finds none — and the kubelet never receives the
reply.
...
```
Failed to scrape node: Get "https://192.0.2.31:10250/metrics/resource":
...
kubectl exec <pod> -- nc -zv 192.0.2.31 10250
```
...
- Traefik (2 replicas, `192.0.2.36` assigned by MetalLB)
- kube-state-metrics
