...
for node in cluster-node-01 cluster-node-02 cluster-node-03 cluster-node-04 cluster-node-05; do
    scp spark-s3-3.5.3.tar eduardo@${node}.internal.example:/tmp/
    ssh eduardo@${node}.internal.example "sudo k3s ctr images import /tmp/spark-s3-3.5.3.tar"
    ssh eduardo@${node}.internal.example "rm /tmp/spark-s3-3.5.3.tar"
done
```
...
2. Uncomment S3 event logging configuration:
   ...
   sparkConf:
     ...
     spark.hadoop.fs.s3a.endpoint: "http://minio.internal.example:9000"
...
- Test from a pod: `kubectl run -n spark test --rm -it --image=curlimages/curl -- curl http://minio.internal.example:9000`
- Check DNS resolution: `host minio.internal.example`
