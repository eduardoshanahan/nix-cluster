...
1. **MinIO S3 Bucket Setup**
   ...
   ```bash
   # Access MinIO UI at s3.internal.example
...
2. **Update nix-cluster-private**
   ...
   ```nix
   homelab.spark.minioEndpoint = "s3.internal.example";
...
Test S3 endpoint:
```bash
kubectl exec -n spark <history-pod> -- curl http://s3.internal.example:9000
```
...
kubectl run -n spark -it --rm s3-test --image=amazon/aws-cli --restart=Never -- \
  s3 --endpoint-url=http://s3.internal.example:9000 ls s3://spark-homelab/
```
